...
'''
1. 캐나다 날씨 데이터에 대하여 CV를 이용하여 
푸리에 기저 갯수를 선택한 결과 K=13이 선택되었다. 퀘벡(l=10) 지역에서도 동일한가?
'''
## 캐나다 날씨 데이터
...
'''
퀘벡 지역의 경우 K값은 5로 택해진다.
'''


'''
2. CD4 데이터에 대하여 fPCA에서 데이터의 변동성이 99% 이상 설명되도록 주성분들을 갯수를 결정하시오. 
이 때 선택된 고유함수들을 시각화하고 환자별 CD4 추정 곡선(몇 개만)을 시각화 하시오.
'''
# CD4 데이터
...
fpca.res$npc
'''데이터 총 분산이 99% 이상 설명되도록하는 PCs 수는 4개'''
...
legend("bottomright", lwd=2, lty=1, col=1:3, 
       legend = paste0("fPC", 1:3))
